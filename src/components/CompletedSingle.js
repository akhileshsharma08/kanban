...
const CompletedSingle = ({item,index}) => {
      ...
      >
        <div className="card p-2 shadow">
          ...
          <div className="textofcard px-1">
            ...
            {!item.img?(<span className="text-gray-500 capitalize">{item.work} </span>):(<img src={item.img} alt={item.title} width={'w-full'} height={30} />)}
            
          </div>
          <ul className="flex justify-around items-center my-2 text-gray-500">
              <li className="">
               
                <img src={item.users} alt="user pics" width={60} height={35} />
              </li>
...
export default CompletedSingle